...
RESULTS_DIR = CURRENT_DIR / Path("results/uncertainty_random_forest")
...
def normalized_entropy(probabilities: np.ndarray) -> np.ndarray:
    """
    Computes normalized entropy in [0, 1].
    Values close to 0 indicate confident predictions.
    Values close to 1 indicate high uncertainty.
    """
    eps = 1e-12
...
def expected_calibration_error(y_true: np.ndarray, y_pred: np.ndarray, confidences: np.ndarray, n_bins: int = 10):
    """
