...
# Pesos iniciais da prioridade.
# Depois podemos ajustar empiricamente.
W_UNCERTAINTY = 0.40
W_NOVELTY = 0.40
W_RARITY = 0.20
...
def compute_mahalanobis_novelty(X_eval: np.ndarray, class_models: dict) -> np.ndarray:
    ...
    # Distância mínima até qualquer classe conhecida.
    # Maior distância mínima = mais "novo"/diferente.
...
def compute_rarity_weights(y_train: np.ndarray, predicted_labels: np.ndarray) -> np.ndarray:
    """
    Calcula raridade com base na frequência da classe prevista no treino.
    ...
    rarity = np.asarray(rarity)

    return minmax_scale(rarity)
...
def main():
    ...
    # Define classes raras dentro do conjunto filtrado.
    # Aqui usamos o primeiro quartil da distribuição de frequências.
