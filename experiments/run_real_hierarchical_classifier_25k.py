...
OUTPUT_DIR = ROOT_DIR / "results" / "real_hierarchical_classifier_25k"
...
def main():
    ...
    family_model = make_family_model()
    ...
    for family in sorted(y_train_family.unique()):
        ...
        subtype_model = make_subtype_model()
        subtype_model.fit(X_fam, y_fam, sample_weight=subtype_sample_weight)

        family_models[family] = subtype_model

    print("\nPredicting family for test objects...")

    predicted_family = family_model.predict(X_test)

    family_accuracy = accuracy_score(y_test_family, predicted_family)
    family_macro_f1 = f1_score(y_test_family, predicted_family, average="macro")

    print("\nFamily-stage performance:")
    print(f"Family accuracy: {family_accuracy:.4f}")
    print(f"Family Macro-F1: {family_macro_f1:.4f}")

    print("\nPredicting subtype using predicted family...")

    y_pred = np.empty(len(X_test), dtype=int)

    X_test_reset = X_test.reset_index(drop=True)
    predicted_family_series = pd.Series(predicted_family)

    for family in sorted(predicted_family_series.unique()):
        mask = predicted_family_series == family
        indices = np.where(mask)[0]

        X_group = X_test_reset.loc[indices]

        if family in family_constants:
            y_pred[indices] = family_constants[family]
        else:
            subtype_model = family_models[family]
            y_pred[indices] = subtype_model.predict(X_group).astype(int)
    ...
    print("\nReal hierarchical classification results:")
    ...
    predictions = pd.DataFrame({
        ...
        "predicted_family_stage": predicted_family,
        "family_stage_correct": y_test_family.to_numpy() == predicted_family,
        "predicted_label": y_pred,
        "predicted_class_name": [label_to_name[int(v)] for v in y_pred],
        "predicted_family_final": [label_to_family[int(v)] for v in y_pred],
        "correct": y_true == y_pred,
    })

    metrics = pd.DataFrame([{
        "model": "real_hierarchical_hist_gradient_boosting",
        "n_objects": len(X),
        "n_train": len(X_train),
        "n_test": len(X_test),
    ...
    predictions.to_csv(OUTPUT_DIR / "real_hierarchical_predictions.csv", index=False)
    metrics.to_csv(OUTPUT_DIR / "real_hierarchical_metrics.csv", index=False)

    with open(OUTPUT_DIR / "real_hierarchical_report.txt", "w", encoding="utf-8") as f:
        f.write(report)

    print("\nSaved:")
    print(f"- {OUTPUT_DIR / 'real_hierarchical_predictions.csv'}")
    print(f"- {OUTPUT_DIR / 'real_hierarchical_metrics.csv'}")
    print(f"- {OUTPUT_DIR / 'real_hierarchical_report.txt'}")
