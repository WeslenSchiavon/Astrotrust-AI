...
RESULTS_ROOT = ROOT_DIR / "results" / "large_followup_prioritization_global_rarity"
...
def load_global_rarity(class_counts_path: Path):
    counts = pd.read_csv(class_counts_path)

    required = ["label", "class_name", "n_objects", "rarity_rank"]
    missing = [c for c in required if c not in counts.columns]
    if missing:
        raise ValueError(f"Missing columns in class counts: {missing}")

    counts = counts.sort_values("rarity_rank").copy()

    # Bottom quartile: 25% rarest classes.
...
def compute_global_rarity_weights(
    global_counts: dict,
...
def main():
    ...
    results_dir = RESULTS_ROOT / f"{args.n_objects}obj" / args.calibration
    results_dir.mkdir(parents=True, exist_ok=True)

    global_counts, rare_classes, class_names = load_global_rarity(class_counts_path)

    print(f"\nLoading features from: {features_path}")

    X, y, object_ids, df = prepare_dataset(features_path)
    ...
    print("\nRare classes based on global ELAsTiCC2 distribution:")
    for cls in rare_classes:
        print(f"  label={cls} | class={class_names.get(cls)} | global_count={global_counts.get(cls)}")
    ...
    print(f"Fitting {args.calibration} calibration...")
    ...
    metrics = pd.DataFrame([{
        ...
        "rarity_definition": "global_bottom_quartile",
        "accuracy": accuracy,
    ...
    enrichment.to_csv(results_dir / "followup_enrichment_vs_random.csv", index=False)
