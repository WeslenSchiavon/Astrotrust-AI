...
def format_metric_for_md(value, digits: int = 4):
    try:
...
def dataframe_to_markdown_table(df, columns):
    lines = []
    lines.append("| " + " | ".join(columns) + " |")
    lines.append("|" + "|".join([":--" if col == "model" else "--:" for col in columns]) + "|")
    ...
    for _, row in df.iterrows():
        ...
        for col in columns:
            ...
            if col == "model":
                ...
                values.append(format_metric_for_md(val))
        lines.append("| " + " | ".join(values) + " |")

    return "\n".join(lines)


def write_markdown_summary(output_dir: Path, dataset_size: str, seed: int, split_seed: int, results_df, final_model_name: str):
    md_path = output_dir / f"ensemble_scaling_summary_{dataset_size}.md"

    dominant = results_df.loc[results_df["model"] == final_model_name]
    hybrid = results_df.loc[results_df["model"] == "hybrid_cnn_tabular"]
    lgbm = results_df.loc[results_df["model"] == "lightgbm_baseline"]

    dominant_row = dominant.iloc[0] if len(dominant) else None
    hybrid_row = hybrid.iloc[0] if len(hybrid) else None
    lgbm_row = lgbm.iloc[0] if len(lgbm) else None

    lines = []
    lines.append(f"# AstroTrust-AI Ensemble Scaling Report: {dataset_size}")
    lines.append("")
    lines.append(f"Generated at: `{pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}`")
    lines.append("")
    lines.append("## Execution setup")
    lines.append("")
    lines.append(f"- Dataset size: `{dataset_size}`")
    lines.append(f"- Training seed: `{seed}`")
    lines.append(f"- Split seed: `{split_seed}`")
    lines.append(f"- Final fixed ensemble: `{final_model_name}`")
    lines.append("")
    lines.append("## Final fixed ensemble metrics")
    lines.append("")
    ...
    if dominant_row is not None:
        ...
        for metric in [
            ...
            lines.append(f"| {metric} | {format_metric_for_md(dominant_row.get(metric))} |")
    else:
        lines.append("Final ensemble row not found.")

    lines.append("")
    lines.append("## Gain over base models")
    lines.append("")
    lines.append("| comparison | accuracy gain | macro-F1 gain |")
    lines.append("|:--|--:|--:|")

    if dominant_row is not None and hybrid_row is not None:
        lines.append(
            "| dominant - hybrid_cnn_tabular | "
            f"{float(dominant_row['accuracy']) - float(hybrid_row['accuracy']):+.4f} | "
            f"{float(dominant_row['macro_f1']) - float(hybrid_row['macro_f1']):+.4f} |"
        )

    if dominant_row is not None and lgbm_row is not None:
        lines.append(
            "| dominant - lightgbm_baseline | "
            f"{float(dominant_row['accuracy']) - float(lgbm_row['accuracy']):+.4f} | "
            f"{float(dominant_row['macro_f1']) - float(lgbm_row['macro_f1']):+.4f} |"
        )
    ...
    lines.append(dataframe_to_markdown_table(results_df[display_cols], display_cols))
    lines.append("")
    lines.append("## Interpretation note")
    lines.append("")
    lines.append(
        "This report is a dataset-size scaling check. The fixed `ensemble_hybrid_dominant` "
        "configuration should be used for clean comparison across 25k, 100k, and 250k. "
        "Other ensemble variants in the full comparison table are auxiliary exploratory checks."
    )
    lines.append("")
...
def main():
    ...
    md_summary_path = write_markdown_summary(
